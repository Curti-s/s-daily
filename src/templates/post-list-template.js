        ...
        <div className="w-2/3">
            ...
            {post.author && post.author.map(a =>
              a.image.localFile.childImageSharp ? (
                <Img fluid={a.image.localFile.childImageSharp.fluid} alt={a.image.localFile.childImageSharp.fluid.originalName} className="h-6 w-6 rounded-full border-2 border-gray-900" />
                  ...
                  <img src="https://via.placeholder.com/20" alt="placeholder author-image" className="rounded-full" />
                )
            )}
            {post.author.map(a =>
              <p className="pl-2 text-sm text-black-900 font-semibold">{a.name} <span className="text-black-300 font-medium">in</span> ShrapDaily</p>
            )}
          </div>
          <div className="flex flex-col mr-4">
            ...
            <div className="pt-2">
              ...
              <p className="pt-1 text-xs text-gray-500">{post.date} . {post.content.markdownNode.childMdx.timeToRead} min read</p>
            </div>
          </div>
        </div>
        <div className="1/3">
